...
class VacuumPlanning(Problem):
    ...
    def path_cost(self, curNode, state1, action, state2):
        ...
        if (self.turnCostOn == True):
            totalcost += ( 0.5 * self.computeTurnCost(curNode.action , action))
        return totalcost
    ...
    def computeTurnCost(self, action1, action):
        ...
        if action1 == action:
            return 0
              ...
              (action1 == 'DOWN' and action == 'LEFT') or (action1 == 'LEFT' and action == 'UP') or
              (action1 == 'UP' and action == 'LEFT') or (action1 == 'LEFT' and action == 'DOWN') or 
              (action1 == 'DOWN' and action == 'RIGHT') or (action1 == 'RIGHT' and action == 'UP')):
            return 1
        # We will return 2 for any other case, which means 180' rotation and hence 2 will multiply by 0.5
        else:
            return 2
    ...
    def findMinManhattanDist(self, pos):
        ...
        self.env.dirtyRooms."""
        min_dist = float('inf') # make it infinity so its a very large number 
